	...
	*/
	...
	player.on('audioTracks', function(e) {
		...
		if(audioTracks.length > 1 && !bLoaded) {
		    if(localStorage.getItem('UserDefLang') !== null){
			    for(var index=0;index<audioTracks.length;index++) {
			        if(audioTracks[index].language == localStorage.getItem('UserDefLang')){
			...
			if(player.getCurrentAudioTrack() == -1 && !UseUserDefLang) {
			...
			if(UseUserDefLang || localStorage.getItem('UserDefLang') == null){
			    localStorage.setItem('UserDefLang', audioTracks[e.currentTrack].language);
			    UseUserDefLang = true;
			}
		}
	...
	player.on('error', function() {
        if(localStorage.getItem('UserDefLang') != null){
            if(localStorage.getItem('UserDefLang') == "jpn"){
	           window.parent.postMessage("jap", '*');
            }else{
	           window.parent.postMessage(localStorage.getItem('UserDefLang'), '*');
            }
	    }else{
	       window.parent.postMessage("hin", '*');
	    }
		if(tryCount < 2) {
			tryCount++;
			bLoaded = false;
	...
	// Listen for messages from the parent to set auto-skip flags
	let autoSkip = { intro: false, outro: false };
	let lastSkipTime = { intro: 0, outro: 0 };

    function handleMessage(event) {
      // Process auto-skip settings
      if (event.data && event.data.autoSkip !== undefined) {
        autoSkip = event.data.autoSkip;
      }

      // Process audio language selection
      if (event.data && event.data.audioLanguage !== undefined) {
        const receivedLang = event.data.audioLanguage;

        // Convert the code to full language name and store it
        if (receivedLang) {
          localStorage.setItem('UserDefLang', receivedLang);

          // If player is already loaded, try to set the audio track
          if (player && audioTracks && audioTracks.length > 0) {
            for (let i = 0; i < audioTracks.length; i++) {
              if (audioTracks[i].language === receivedLang) {
                player.setCurrentAudioTrack(audioTracks[i].hlsjsIndex);
                currentAudioTrack = audioTracks[i].hlsjsIndex;
	...
	player.on('time', () => {
	  ...
	  const pos = player.getPosition(), d = player.getDuration();
	  ["intro", "outro"].forEach(seg => {
	    if (videoSettings.fields[seg]) {
	      const [s, e] = videoSettings.fields[seg].split("-").map(Number),
	            ml = (s / d) * 100,
	            w = ((e - s) / d) * 100;

	      if (!$(`.jw-${seg}`).length)
	        $('.jw-slider-container.jw-reset').append(
	          `<div class="jw-reset jw-${seg}" style="position:absolute;margin-left:${ml}%;width:${w}%;background-color:#28c2bd !important;animation:none !important;z-index:3;transform:translate(0,0) scale(1,.6);transition:transform 150ms ease-in-out;"></div>`
	        );

	      if (pos >= s && pos <= e) {
	        if (autoSkip[seg]) {
	          // Only auto-skip if enough time has passed since last skip (3 second delay)
	          const now = Date.now();
	          if (now - lastSkipTime[seg] >= 3000) {
	            player.seek(e);
	            lastSkipTime[seg] = now;
	          }
	        } else {
	          // Show skip button logic remains unchanged
	          if (!$(`#skip-${seg}`).length) {
	            $('.jw-wrapper').prepend(
	              `<div class="jw-reset jw-skip-${seg}"><div class="boxRight"><a href="javascript:;" id="skip-${seg}" class="iBtn iBtn-outline">Skip ${seg.charAt(0).toUpperCase() + seg.slice(1)}</a></div></div>`
	            );
	            $(`#skip-${seg}`).off("click").click(() => {
	              player.seek(e);
	              $(`.jw-skip-${seg}`).remove();
	            });
	          }
